        ...
        <AnimatePresence mode="wait">
          ...
          {plan === "smallgroup" && sgUnit === "cohama" && (
            <motion.div key="sg-cohama" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}>
              ...
              <div className="flex gap-2 mb-10 overflow-x-auto pb-2 scrollbar-none justify-start sm:justify-center px-1 snap-x snap-mandatory">
                {periods.map(({ key, label, note }) => (
                  <button
                    key={key}
                    onClick={() => setPeriod(key)}
                    className={`flex-shrink-0 flex flex-col items-center px-4 sm:px-5 py-2 rounded-full text-xs sm:text-sm font-bold border transition-all snap-start ${period === key ? "bg-primary text-primary-foreground border-primary" : "border-border text-secondary-foreground hover:border-primary/50"}`}
                  >
                    {label}
                    <span className={`text-[9px] sm:text-[10px] font-normal mt-0.5 ${period === key ? "text-primary-foreground/70" : "text-secondary-foreground/60"}`}>{note}</span>
                  </button>
                ))}
              </div>
